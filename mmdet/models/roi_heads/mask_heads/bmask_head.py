...
class BMaskHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.loss_boundary = build_loss(loss_boundary)
        ...
        self.b_convs = nn.ModuleList()
        for i in range(self.b_num_convs):
            in_channels = (
                self.in_channels if i == 0 else self.conv_out_channels)
            padding = (self.conv_kernel_size - 1) // 2
            self.b_convs.append(
        ...
        if self.upsample_method is None:
            ...
            self.b_upsample = build_upsample_layer(upsample_cfg_)
        ...
        self.b_conv_logits = Conv2d(logits_in_channel, 1, 1)
    ...
    @auto_fp16()
    def forward(self, x, b_x):
        for conv in self.convs:
            x = conv(x)
        
        b_x = self.b_downsample(b_x)

        b_x = b_x + self.to_b_conv(x)

        for conv in self.b_convs:
            b_x = conv(b_x)
        
        x = self.b_to_conv(b_x)
        
        x = self.fusion(x)
            
        if self.upsample is not None:
            x = self.upsample(x)
            b_x = self.b_upsample(b_x)
            if self.upsample_method == 'deconv':
                x = self.relu(x)
                b_x = self.relu(b_x)
        mask_pred = self.conv_logits(x)
        boundary_pred = self.b_conv_logits(b_x)
    ...
    @force_fp32(apply_to=('mask_pred', 'boundary_pred',))
    def loss(self, mask_pred, boundary_pred, mask_targets, labels):
        loss = dict()
        if mask_pred.size(0) == 0:
            loss_mask = mask_pred.sum()
            loss_boundary = boundary_pred.sum()
        else:
            if self.class_agnostic:
                loss_mask = self.loss_mask(mask_pred, mask_targets,
                                           torch.zeros_like(labels))
                loss_boundary = self.loss_boundary(boundary_pred, mask_targets,
                                           torch.zeros_like(labels))
            else:
                loss_mask = self.loss_mask(mask_pred, mask_targets, labels)
                loss_boundary = self.loss_boundary(boundary_pred, mask_targets, labels)
